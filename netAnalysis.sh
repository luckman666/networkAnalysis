...
bit_to_human_readable(){
    ...
    if [[ ${trafficValue%.*} -gt 922 ]];then
        ...
        if [[ ${trafficValue%.*} -gt 922 ]];then
            #conv to Mb
            trafficValue=`awk -v value=$trafficValue 'BEGIN{printf "%0.1f",value/1024}'`
            echo -ne "${trafficValue}Mb \n"
        else
            echo -ne "${trafficValue}Kb \n"
        fi
    else
        echo -ne "${trafficValue}b \n"
    fi
}
...
trafficAndConnectionOverview(){
    ...
    echo -ne "\033[32mnetwork device $eth average traffic in 10s: \033[0m \n"
    ...
    #统计每个端口在10s内的平均流量
    echo -ne "\033[32maverage traffic in 10s base on server port: \033[0m \n"
    ...
    echo -ne "\033[32mtop 10 ip average traffic in 10s base on server: \033[0m \n"
    ...
    echo -ne "\033[32mconnection state count: \033[0m \n"
    awk 'NR>1{sum[$(NF-4)]+=1}END{for (state in sum){print state,sum[state]}}' /tmp/ss | sort -k 2 -nr
    echo
    #统计各端口连接状态
    echo -ne "\033[32mconnection state count by port base on server: \033[0m \n"
    ...
    echo -ne "\033[32mtop 10 ip ESTAB state count at port 80: \033[0m \n"
    ...
    echo -ne "\033[32mtop 10 ip SYN-RECV state count at port 80: \033[0m \n"
