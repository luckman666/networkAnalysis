...
bit_to_human_readable(){
    ...
    if [[ ${trafficValue%.*} -gt 922 ]];then
        ...
        if [[ ${trafficValue%.*} -gt 922 ]];then
            #conv to Mb
            trafficValue=`awk -v value=$trafficValue 'BEGIN{printf "%0.1f",value/1024}'`
            echo -e "${trafficValue}Mb \n"
        else
            echo -e "${trafficValue}Kb \n"
        fi
    else
        echo -e "${trafficValue}b \n"
    fi
}
...
trafficAndConnectionOverview(){
    ...
    echo -e "please wait for 10s to generate network data... \n"
    echo
    #当前流量值
    ...
    echo -e "$eth Receive: $(bit_to_human_readable $eth_in)/s \n"
    echo -e "$eth Transmit: $(bit_to_human_readable $eth_out)/s \n"
    echo
